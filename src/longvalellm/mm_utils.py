...
class SpeechExtractor():
    def __init__(self, model, n_chunks=100, sampling_rate=16000):
    ...
    def extract(self, data):
        ...
        if len(waveform) > 30 * self.sampling_rate:
            audio_list = [waveform[i: i + 30 * self.sampling_rate] for i in range(0, len(waveform), 30 * self.sampling_rate)]
            spectrogram_list = []
