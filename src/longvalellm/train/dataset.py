...
from longvalellm.constants import IGNORE_INDEX, IMAGE_TOKEN_INDEX, DEFAULT_IMAGE_TOKEN
from longvalellm import conversation as conversation_lib
from longvalellm.mm_utils import tokenizer_image_token
...
class DataArguments:
    ...
    asr_feat_folder: Optional[str] = field(default=None)
...
def preprocess_v1(
    ...
    if input_ids.shape[1] > tokenizer.model_max_length:
        ...
        import re
        if len(sources[0]) == 2: #single-turn
            pattern = r"From \d+ to \d+,"
        else: 
            pattern = conv.sep2
        
        matches = list(re.finditer(pattern, conversations[0]))
        conv_rounds = []
        start = 0
        for match in matches:
            end = match.start()
            if start < end:
                conv_rounds.append(conversations[0][start:end].strip())
            # conv_rounds.append(match.group().strip())
            start = end
        if start < len(conversations[0]):
            conv_rounds.append(conversations[0][start:].strip())

        # else:  #multi-turn
            # conv_rounds = conversations[0].split(conv.sep2)[:-1]

        truncated_conversation = conversations[0]
        while input_ids.shape[1] > tokenizer.model_max_length and len(conv_rounds) > 1:
            conv_rounds = conv_rounds[:-1]
            truncated_conversation = ' '.join(conv_rounds) 
            conversations[0] = truncated_conversation + conv.sep2 #</s>
            if has_image:
                input_ids = torch.stack([tokenizer_image_token(prompt, tokenizer, return_tensors='pt') for prompt in conversations], dim=0)
...
class LazySupervisedDataset(Dataset):
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        try:
            audio_feature_path = '{}/{}.npy'.format(self.data_args.audio_feat_folder, source['id'])
            audio = np.load(audio_feature_path)
            audio = torch.from_numpy(audio)
        except:
            audio = None

        try:
            asr_feature_path = '{}/{}.npy'.format(self.data_args.asr_feat_folder, source['id'])
            asr = np.load(asr_feature_path)
            asr = torch.from_numpy(asr)
        except:
            asr = None
...
class DataCollatorForSupervisedDataset(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        if 'audio' in instances[0]:
            batch_audio = [instance['audio'] for instance in instances]
            if all(x is not None and x.shape == batch_audio[0].shape for x in batch_audio):
                batch['audio'] = torch.stack(batch_audio)
            else:
                batch['audio'] = batch_audio
        
        if 'asr' in instances[0]:
            batch_asr = [instance['asr'] for instance in instances]
            if all(x is not None and x.shape == batch_asr[0].shape for x in batch_asr):
                batch['asr'] = torch.stack(batch_asr)
            else:
                batch['asr'] = batch_asr
