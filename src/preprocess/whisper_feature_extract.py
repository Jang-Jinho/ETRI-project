...
class AudioDataset(Dataset):
    def __init__(self, annotation, audio_dir, model):
...
# DataLoader
def create_data_loader(annotation, audio_dir, model, batch_size=1, num_workers=4):
    assert batch_size == 1, "batch_size must be 1"
    dataset = AudioDataset(annotation, audio_dir, model)
...
if __name__ =='__main__':
    ...
    parser.add_argument("--save_dir", type=str, default="./data/features_eval/speech_features")
    ...
    second_per_window = 2.56 # 5.12
    second_stride = 2.56 # 5.12
    with torch.no_grad():
